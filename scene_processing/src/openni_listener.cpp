...
void lookForClassInOriginalFrameProjectiveMargin(vector<int> & classes, pcl::PointCloud<pcl::PointXYZRGBCamSL> & cloud, vector<SpectralProfile> & spectralProfiles, map<int, int> & segIndex2label, const std::vector<pcl::PointCloud<PointT> > &segment_clouds, int scene_num, vector<pcl::PointXYZI> & maximas)
{
    int imageWidth = 640;
    int imageHeight = 480;

    pcl::PointXYZ steps(0.01, 0.01, 0.1);
    std::vector< pcl::KdTreeFLANN<PointT>::Ptr > trees;
    ...
    getMaxRanges(maxDist, cloud);
    //double cost;
    double maxMargin[classes.size()];
    double minMargin[classes.size()];

    double cost[NUM_CLASSES];
    double maxCost;
    ...
    for (int oclass = 0; oclass < classes.size(); oclass++)
    {
        minMargin[oclass] = DBL_MAX;
        maxMargin[oclass] = -DBL_MAX;
        heatMapTop[oclass].setConstant(imageHeight, imageWidth, -FLT_MAX);
        // heatMapFront[oclass].setConstant(numBins[2], numBins[1], -FLT_MAX);
    }
        ...
        for (y = min.y, county = 0; y < max.y; county++, y += steps.y)
            for (z = min.z, countz = 0; z < max.z; countz++, z += steps.z)
            {
                ...
                for (int k = 0; k < NUM_CLASSES; k++)
                {
                    ...
                    cost[k] += nodeFeatsB.dot(nodeWeights->row(k));
                    ...
                    for (size_t i = 0; i < neighbors.size(); i++)
                    {
                        ...
                        cost[k] += edgeFeatsB.dot(edgeWeights[k]->row(nbrLabel));
                        ...
                        assert(k >= 0);
                        assert(k < NUM_CLASSES);
                        cost[k] += edgeFeatsB.dot(edgeWeights[nbrLabel]->row(k));
                    }
                ...
                for (int oclass = 0; oclass < classes.size(); oclass++)
                {
                    ...
                    double margin = cost[classes.at(oclass)] - maxCost;

                    //     cout<<x<<","<<y<<","<<z<<","<<dist<<","<<cost<<endl;
                    if (maxMargin[oclass] < margin)
                    {
                        maxMargin[oclass] = margin;
                        maxS[oclass].setCentroid(target);
                    }
                    else if (maxMargin[oclass] == margin)
                    {
                        maxS[oclass].addCentroid(target);
                    }

                    if (minMargin[oclass] > margin)
                    {
                        minMargin[oclass] = margin;
                    }

                    projectPointToImage(x, y, z, imageY, imageX);
                    if (imageX >= 0 && imageY >= 0 && imageX < imageWidth && imageY < imageHeight && heatMapTop[oclass](imageY, imageX) < margin)
                    {
                        heatMapTop[oclass](imageY, imageX) = margin;
                    }
    ...
    for (size_t oclass = 0; oclass < classes.size(); oclass++)
    {
        ...
        maximas[oclass].y = maxS[oclass].centroid.y;
        maximas[oclass].z = maxS[oclass].centroid.z;
        maximas[oclass].intensity = maxMargin[oclass];

        writeHeatMap<float>((lexical_cast<string > (classes[oclass]) + "_topHeat" + lexical_cast<string > (scene_num) + ".png").data(), heatMapTop[oclass], maxMargin[oclass], minMargin[oclass], imageY, imageX);
        //  writeHeatMap<float>((lexical_cast<string > (classes[oclass]) + "frontHeat" + lexical_cast<string > (scene_num) + ".png").data(), heatMapFront[oclass], maxCost[oclass], minCost[oclass], numBins[2] - 1 - getBinIndex(maxS[oclass].getCentroid(), min, steps, 2), getBinIndex(maxS[oclass].getCentroid(), min, steps, 1));
        //cout << "optimal point" << maxS.centroid.x << "," << maxS.centroid.y << "," << maxS.centroid.z << " with cost:" << minCost << endl;
    }
    exit(1);
...
void getMovement(bool lookFor){
    ...
    if (lookFor) {
        ...
        while (cloudVector.size() > 0) {
            ...
            lookForClassInOriginalFrameProjectiveMargin(labelsToLookFor, cloudVector.at(0), spectralProfilesVector.at(0), segIndex2LabelVector.at(0), segment_cloudsVector.at(0), sceneNumVector.at(cloudCount), frame_maximas);
